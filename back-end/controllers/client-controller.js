...
exports.search = async (req, res) => {
  try {
    const { searchItem, mode } = req.body;

    let AllCoins, AllCurrs;

    if (mode === "top") {
      const [TopCoins, TopCurrs] = await Promise.all([
        CoinPrice.findAll({
          include: {
            model: Coin,
          },
      ...
      AllCoins = TopCoins.filter((coin) =>
        coin.Coin.name.toLowerCase().includes(searchItem.toLowerCase())
      );
      AllCurrs = TopCurrs.filter(
        (curr) =>
          curr.Currency.name.toLowerCase().includes(searchItem.toLowerCase()) ||
          curr.Currency.symbol.toLowerCase().includes(searchItem.toLowerCase())
      );
    } else if (mode === "all") {
      [AllCoins, AllCurrs] = await Promise.all([
        CoinPrice.findAll({
          include: {
            model: Coin,
      ...
      return res
        .status(400)
        .json({ error: "Invalid mode. Use 'top' or 'all'." });
    }
